...
class CalculadoraIP:
    def __init__(self, master):
        ...
        self.calcular_button = tk.Button(
            master, text="Calcular", command=self.calcular_subredes,
        ...
        self.resultados_text = tk.Text(
            self.resultados_frame, height=15, width=70, yscrollcommand=self.scrollbar.set,
    ...
    def calcular_subredes(self):
        ...
        try:
            ...
            num_hosts = rede.num_addresses

            if num_hosts > 2:
                primeiro_host = endereco_rede + 1
                ultimo_host = endereco_broadcast - 1
                qtd_hosts_validos = num_hosts - 2
            else:
                primeiro_host = ultimo_host = "N/A"
                qtd_hosts_validos = 0

            primeiro_octeto = int(str(endereco_rede).split('.')[0])
            ...
            num_subredes = 2 ** bits_subrede if bits_subrede >= 0 else 0
            ...
            self.resultados_text.insert(tk.END, f"Hosts válidos por Sub-rede: {qtd_hosts_validos}\n")
